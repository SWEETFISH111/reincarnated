...
public class Usurper {
    public static UUID getUsurper(MagiculeCircuit circuit) {

        MagiculeCircuit.NodeData triggerNode = new MagiculeCircuit.NodeData(UUID.randomUUID(), MagiculeNodeType.ON_ATTACK_STRONGER, 100, 100);
        MagiculeCircuit.NodeData conversNode = new MagiculeCircuit.NodeData(UUID.randomUUID(), MagiculeNodeType.COMBERS_POWERGAP_TO_MASO, 150, 100);
        ...
        return circuit.collapseNodes(nodes, Component.translatable("name.reincarnated.uniqueSkill.usurper").getString());
    }
}
